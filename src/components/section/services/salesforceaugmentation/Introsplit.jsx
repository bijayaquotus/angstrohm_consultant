...
export default function IntroSplit({ eyebrow, heading, paragraphs = [], ctaText, ctaHref = '#', image }) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Animated border glow on image container
      gsap.to('.image-glow', {
        scrollTrigger: {
          trigger: sectionRef.current,
          start: "top 70%",
    ...
    <section ref={sectionRef} className="bg-[#F4F1EA] py-16 md:py-20 px-6 overflow-hidden relative">
      ...
      <div className="max-w-[1280px] mx-auto grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-16 items-center relative z-10">
        ...
        <div className="md:col-span-5 flex justify-center relative">
          {/* Image glow effect */}
          <div className="image-glow absolute -inset-4 bg-gradient-to-r from-[#c8385a]/20 via-[#5fb8e0]/20 to-[#c8385a]/20 rounded-2xl blur-2xl opacity-0" />
          
          <img 
            ref={imageRef}
            src='/services/sales-force-success.png' 
            alt={heading} 
            className="w-full max-w-sm rounded-sm transition-all duration-300 relative"
          />
        </div>
