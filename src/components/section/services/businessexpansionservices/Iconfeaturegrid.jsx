          ...
          {items.map((item, i) => {
            ...
            return (
              <div key={i} className="w-full sm:w-[280px]">
                <div className="w-14 h-14 rounded-md border border-[#3b4a8f]/60 bg-[#161544] flex items-center justify-center mb-10">
                  {Icon && <Icon className="w-full h-full text-[#5fb8e0]" strokeWidth={1.5} />}
                </div>
                <h3 className="text-[#c8385a] font-bold text-sm tracking-wide uppercase mb-3">
                  {item.title}
                </h3>
                <p className="text-slate-300 text-sm-3 leading-relaxed">
                  {item.description}
                </p>
              </div>
            );
